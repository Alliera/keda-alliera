...
var urlPaths = []string{"/sub1/sub2/sub3/myHost", "/sub1/sub2/myHost", "/sub1/myHost", "/sub1/sub2/", "/sub1/", "/", ""}
...
func TestGetQueueInfo(t *testing.T) {
	...
	for _, testData := range allTestData {
		...
		path, vhost := getVhostAndPathFromURL(testData.urlPath, testData.extraMetadata["vhostName"])

		var apiStub = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			expectedPath := fmt.Sprintf("%s/api/queues%s/evaluate_trials", path, vhost)
			uri := strings.ReplaceAll(r.RequestURI, "//", "/")
			if uri != expectedPath {
				t.Error("Expect request path to =", expectedPath, "but it is", uri)
			}
